...
def test_preserve_quarterly_pattern():
    ...
    for ivl, expected_month in zip(intervals, expected_months):
        dt = datetime.fromtimestamp(ivl.start, tz=timezone.utc)
        assert dt.month == expected_month, f"Expected month {expected_month}, got {dt.month}"
...
def test_preserve_complex_pattern():
    ...
    for ivl in intervals:
        dt = datetime.fromtimestamp(ivl.start, tz=timezone.utc)
        assert dt.month in [3, 6, 9, 12], f"Expected March/Jun/Sep/Dec, got month {dt.month}"
...
def test_preserve_timezone():
    ...
    for ivl in intervals:
        ...
        if dt_pacific >= query_start_pacific:
            assert dt_pacific.hour == 9, f"Expected 9 AM Pacific, got {dt_pacific.hour} for {dt_pacific}"
            assert dt_pacific.minute == 0, f"Expected 0 minutes, got {dt_pacific.minute}"
